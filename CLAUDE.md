...
- More configurable slots (same page): toolbar title/cards-due subtitle, right-pane deck name, pane divider colour + width (0 = hidden, `ResizingDivider`), settings icons/toggles/sliders/screen headers; sync icon tinted via its `SyncActionProvider`. The UI page is organized into sections with yellow divider lines (`sk_preference_category` layout) and double-indented items under subheadings (`sk_preference_subheader`/`sk_preference_indented`). Colours are picked with four RGBA sliders + live preview. Settings rows are re-styled on every draw (change-guarded) because preference rebinds (header highlight) restore theme colours.
