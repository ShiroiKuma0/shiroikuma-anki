...
| versionName | `<upstream>+<fork build>`, e.g. `2.24.0+1` |
| versionCode | upstream scheme `AbbCCtDD` with the `DD` digits as the fork build counter, e.g. `22400301` |
...
| Branches | `main` mirrors `upstream/main` (no fork work); `custom` = fork commit stack, rebased onto each upstream release tag (`vX.Y.Z`) |
