...
- No crash-report tracker. Upstream reports crashes with ACRA, which tracker scanners flag: it uploads the stack trace, 500 lines of logcat, the shared preferences, device details and an install UUID to `ankidroid.org/acra/report`. Removed entirely — the four `ch.acra:*` dependencies, `AcraCrashReporter`, `AcraAnalyticsInteraction`, the `:acra` sender process, `AnkiDroidCrashReportDialog` with its `dialog_feedback` layout, the `ACRA_URL` build field and `ACRATest`. `com.ichi2.anki.shiroikuma.ShiroikumaCrashReporter` implements upstream's `CrashReporter` interface inertly (log only, analytics hit kept for the opt-in path), so every `CrashReportService` call site still compiles; `AnkiDroidApp` calls `initializeCrashReporter()`. The two pieces of UI that only fed ACRA went with it: "Error reporting mode" in General settings and "Send exception report" in Help → Get help. Upstream's incidental uses of ACRA utilities were replaced — a local Java-serialization deep copy in `MultimediaEditableNote`, and the ACRA UUID line dropped from `DebugInfoService`. `ShiroikumaNoTrackersTest` fails if a rebase brings `org.acra` back, which is the likely way it would return.
...
- 保存復元 automation contract **v2** (`StateExportReceiver`, `AutomationProvider`, `AutomationDataService`, `AutomationCallers`, `AutomationJobs`, `AutomationProgress`, `AutomationAuth`): the exported surface every sister app implements so 自由作業盤's 保存復元 project backs them all up headlessly, and 応用管理 can back this app up *with its data* and restore it onto a wiped phone. **Broadcast half (§1):** `shiroikuma.anki.action.EXPORT_STATE` runs the panel's own export with no UI (extras `token`/`path`/`items`/`progress_action` + the reply trio `reply_action`/`reply_package`/`reply_id`); `…action.LIST_CATEGORIES` enumerates `id<TAB>label<TAB>parent<TAB>on|off` lines, `collection.media` carrying `collection` as its parent and the only item that starts `off` (largest part of the export, re-obtainable by syncing); `…action.CANCEL_EXPORT` raises the flag the export polls, deletes the partial file and answers the *export's* request with `ERROR:cancelled` — the cancel itself gets no reply at all, refusal included, and is a silent no-op when nothing is running. An absent `items` means our **default set** (every category, no media), not everything. **Data door (§2a):** a `ContentProvider` at `shiroikuma.anki.automation`, exported with no permission, `call()`-only, methods `describe`/`export`/`import`/`cancel`; the payload moves through a `ParcelFileDescriptor` the caller supplies (`dup()`ed before the call returns, closed in a `finally`), never a path. `import` exists **only** here — the receiver is unauthenticated, so an import there would let any app wipe the collection. Callers are pinned by exact package name + uid + signing certificate (`AutomationCallers`); a prefix is not an identity. Long work runs in `AutomationDataService`, which goes foreground **before any early return** (once `startForegroundService` is called the platform demands it, so a stale job id would otherwise crash the app being backed up) and holds a partial wakelock. **Gate:** `automation_enabled` defaults **true**, `automation_require_token` defaults **false**, and a token sent to an app not asking for one is **ignored, never refused**; all three live in `sk_automation` (never exported) and are written with `commit()`, since a lost write would now fail *open*. Three rows in the Export / Import section, the token row shown only when it is required. Replies are fresh broadcasts (never a binder, never the ordered result — EMUI severs both), exactly one per request. Progress goes through the single shared `AutomationProgress` — real counts never a percentage, throttled to 500 ms, with a 25 s heartbeat, correlation id in both `reply_id` and `job_id`. `<queries>` names **both** callers: without it a reply's `setPackage()` fails silently on Android 11+ *and* the certificate check cannot read the caller at all. Covered by `ShiroikumaAutomationTest`.
